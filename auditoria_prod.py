...
def leer_datos_seguro(nombre_hoja, fila_encabezado=0):
    try:
        ...
        df.columns = [
            str(n).strip().upper() if n else f"COL_{i}" for i, n in enumerate(nombres)
        ]
...
# --- BUSCADOR DINÁMICO Y FILTRADO ESTRATÉGICO (BDD) ---
col_sub_actual = ""
col_area_bdd = ""

if not df_bdd_raw.empty:
    # 1. Detectar columna de Subproceso (Cualquiera que diga SUB...CESO)
    for c in df_bdd_raw.columns:
        if "SUB" in c and "CESO" in c:
            col_sub_actual = c
            break
    if not col_sub_actual:
        col_sub_actual = "SUB PRO CESO"

    # 2. Detectar columna de Área/Proceso (Cualquiera que diga AREA o PROCESO)
    for c in df_bdd_raw.columns:
        if c in ["PROCESO", "ÁREA", "AREA"]:
            col_area_bdd = c
            break

    # 3. Filtrado por COL_4 (Solo procesos Activos = TRUE)
    # Lo hacemos robusto para que lea texto o booleano de Sheets
    if "COL_4" in df_bdd_raw.columns:
        df_bdd_raw = df_bdd_raw[
            df_bdd_raw["COL_4"].astype(str).str.upper().str.contains("TRUE", na=False)
        ].copy()

    # 4. Limpieza de datos vacíos o ceros en la BDD
    df_bdd_raw = df_bdd_raw[df_bdd_raw[col_sub_actual].str.len() > 1].copy()
    df_bdd_raw = df_bdd_raw[df_bdd_raw[col_sub_actual] != "0"].copy()

# Sidebar
# --- 2. INTERFAZ: SIDEBAR Y LOGICA DE CORTE ---
with st.sidebar:
    if os.path.exists(LOGO_FILENAME):
        st.image(LOGO_FILENAME, use_container_width=True)

    st.markdown(
        "<h3 style='text-align: center;'>CONTROL DE ACCESO</h3>", unsafe_allow_html=True
    )
    ...
    if not df_programa.empty and "ÁREA" in df_programa.columns:
        lista_areas = [
            a
            for a in df_programa["ÁREA"].unique().tolist()
            if a and a.upper() != "ÁREA"
        ]
    else:
    ...
    # --- INICIO LÓGICA DE CORTE AUTOMÁTICO ---
    ahora = datetime.now(pytz.timezone("America/Mexico_City")).time()

    # Definimos los umbrales de tiempo para sugerir el corte
    if ahora < datetime.strptime("11:30", "%H:%M").time():
        default_index = 0  # Sugiere 11:00 AM
    elif ahora < datetime.strptime("14:30", "%H:%M").time():
        default_index = 1  # Sugiere 14:00 PM
    else:
        default_index = 2  # Sugiere 17:00 PM

    cortes_dict = {"11:00 AM (3h)": 3, "14:00 PM (6h)": 6, "17:00 PM (9h)": 9}
    lista_cortes = list(cortes_dict.keys())

    # El index=default_index es lo que hace la magia
    ...
    # --- FIN LÓGICA DE CORTE AUTOMÁTICO ---

    st.divider()
    # ... (continúa el resto de tu código de Plan del Día)
    st.markdown("### 📋 Plan del Día")
    df_plan_dia = pd.DataFrame()
    if not df_programa.empty:
        df_plan_dia = df_programa[
            (df_programa["FECHA"] == fecha_sel) & (df_programa["ÁREA"] == area_sel)
        ].copy()
        if area_sel.upper() == "MOLDEO" and not df_plan_dia.empty:
            df_plan_dia = df_plan_dia[
                df_plan_dia["PIEZA"].str.contains(
...
if not df_plan_dia.empty and not df_bdd_raw.empty:
    df_sub_base_total = df_bdd_raw[df_bdd_raw["PROCESO"] == area_sel][
        ["PIEZA", col_sub_actual]
    ].copy()
    ...
    if not df_base.empty:
        df_base = pd.merge(
            df_base, df_plan_dia[["PIEZA", "TOTAL"]], on="PIEZA", how="left"
        )
        df_base["TOTAL"] = pd.to_numeric(df_base["TOTAL"], errors="coerce").fillna(0)
        if not df_auditorias.empty:
            df_aud_hoy = df_auditorias[
                (df_auditorias["FECHA"] == fecha_sel)
                & (df_auditorias["ÁREA"] == area_sel)
            ].copy()
            df_aud_hoy["REAL"] = pd.to_numeric(
                df_aud_hoy["REAL"], errors="coerce"
            ).fillna(0)
            df_max_real = (
                df_aud_hoy.groupby(["PIEZA", "SUBPROCESO"])["REAL"].max().reset_index()
            )
            df_final = pd.merge(
                df_base,
                df_max_real,
                left_on=["PIEZA", col_sub_actual],
                right_on=["PIEZA", "SUBPROCESO"],
                how="left",
            ).fillna(0)
        else:
        ...
        df_resumen_final = df_final[
            ["PIEZA", col_sub_actual, "TOTAL", "REAL", "% REAL"]
        ].copy()
        df_resumen_final.columns = [
            "PIEZA",
            "SUBPROCESO",
            "PROGRAMADO",
            "AVANCE",
...
with k2:
    st.markdown(
        f"<div class='metric-card'><small>META TURNO</small><h2>{int(df_resumen_final['PROGRAMADO'].sum()) if not df_resumen_final.empty else 0} PZS</h2></div>",
        unsafe_allow_html=True,
    )
with k3:
    st.markdown(
        f"<div class='metric-card'><small>REAL TURNO</small><h2>{int(df_resumen_final['AVANCE'].sum()) if not df_resumen_final.empty else 0} PZS</h2></div>",
        unsafe_allow_html=True,
    )
...
# --- 1. REGISTRO DE AUDITORÍA (VERSIÓN ROBUSTA ANTI-KEYERROR) ---
st.markdown("<div class='capture-container'>", unsafe_allow_html=True)
st.subheader("🚀 REGISTRO DE AUDITORÍA")

# Identificar dinámicamente la columna de Proceso/Área en la BDD
col_proceso_bdd = next(
    (c for c in df_bdd_raw.columns if c in ["PROCESO", "ÁREA"]), None
)

piezas_validas = df_plan_dia["PIEZA"].unique() if not df_plan_dia.empty else []
piezas_pendientes = []

if not df_plan_dia.empty and not df_bdd_raw.empty and col_proceso_bdd:
    for p in piezas_validas:
        # Filtro seguro para subprocesos
        mask = (df_bdd_raw["PIEZA"] == p) & (df_bdd_raw[col_proceso_bdd] == area_sel)
        subs_totales = df_bdd_raw[mask][col_sub_actual].unique()

        reps_p = (
            df_auditorias[
                (df_auditorias["FECHA"] == fecha_sel)
                & (df_auditorias["CORTE"] == corte_sel)
                & (df_auditorias["PIEZA"] == p)
            ]["SUBPROCESO"].tolist()
            if not df_auditorias.empty
            else []
        )
...
with c1:
    ...
    # FILTRO ROBUSTO PARA df_s (Evita el KeyError que reportaste)
    if col_proceso_bdd and not df_bdd_raw.empty:
        df_s = df_bdd_raw[
            (df_bdd_raw["PIEZA"] == p_sel) & (df_bdd_raw[col_proceso_bdd] == area_sel)
        ].copy()
    else:
        df_s = pd.DataFrame()
        if not col_proceso_bdd:
            st.error(
                "⚠️ No se encontró la columna 'PROCESO' o 'ÁREA' en la pestaña BDD."
            )
    ...
    sub_list = (
        [s for s in df_s[col_sub_actual].unique() if s not in reps]
        if not df_s.empty
        else []
    )
    s_sel = st.selectbox(
        "SUB-PROCESO ⚙️",
        sub_list if sub_list else ["✅ PIEZA TERMINADA"],
        help="Operación a auditar.",
    )

with c2:
    ops = st.number_input("OPERADORES 👥", 1, key=f"ops_{f_id}")
    real = st.number_input("CANTIDAD REAL 🔢", 0, key=f"r_{f_id}")

with c3:
    mins = st.number_input("MIN. PARO ⏳", 0, key=f"m_{f_id}")
...
if s_sel and s_sel != "✅ PIEZA TERMINADA":
    try:
        # Validación de meta segura
        if not df_s.empty and s_sel in df_s[col_sub_actual].values:
            pz_h_val = df_s[df_s[col_sub_actual] == s_sel]["PZ X H"].iloc[0]
            pz_h = float(pz_h_val) if pz_h_val else 0
            meta = int((pz_h * max(0, horas_acum - (mins / 60))) * ops)
            st.info(f"💡 **Meta:** {meta} piezas")

            if st.button("💾 GUARDAR REGISTRO"):
                h = datetime.now(pytz.timezone("America/Mexico_City")).strftime(
                    "%H:%M:%S"
                )
                fila = [
                    fecha_sel,
        ...
        st.error(f"Error al calcular o guardar: {e}")
else:
...
st.divider()
titulo_expander = (
    f"📖 CONSULTAR CAPACIDADES (PZ/H) - PIEZA: {p_sel if p_sel else 'NO SELECCIONADA'}"
)
...
if not df_auditorias.empty and not df_programa.empty and not df_bdd_raw.empty:
    ...
    df_p_v["FECHA_DT"] = pd.to_datetime(
        df_p_v["FECHA"], format="%d/%m/%Y", errors="coerce"
    )
    df_p_v = df_p_v[
    ...
    df_p_m = df_p_v[
        mask_m
        & df_p_v["PIEZA"].str.contains("GENERAL|VACIADO|ADOBES", case=False, na=False)
    ]
    ...
    df_max_a = (
        df_a_v.groupby(["FECHA", "PIEZA", "SUBPROCESO"])["REAL"].max().reset_index()
    )
    ...
    col_sub_v = next(
        (c for c in df_bdd_v.columns if "SUB" in c and "CESO" in c), "SUB PRO CESO"
    )

    # 6. Cruce Maestro
    df_base_v = pd.merge(
        df_p_final[["FECHA", "ÁREA", "PIEZA", "TOTAL"]],
        df_bdd_v[["PIEZA", col_sub_v, "PROCESO"]],
        on="PIEZA",
    )
    ...
    df_unificado_v["TOTAL"] = pd.to_numeric(
        df_unificado_v["TOTAL"], errors="coerce"
    ).fillna(0)
    # Eliminamos el .clip(upper=100)
    df_unificado_v["% REAL"] = (
        df_unificado_v["REAL"] / df_unificado_v["TOTAL"] * 100
    ).fillna(0)
